...
public class BEventAtTime extends BComponent
{
  ...
  {            
    ...
    BAbsTime nextEvent = BAbsTime.make(BAbsTime.make(now.getYear(),now.getMonth(),now.getDay()),getEventTimeOut());
    ...
    else
    {
      ...
      //going to set an arbitrary threshold of 1 minute.  
      //If the times are within one minute of each other, fire the event.  
      //Otherwise, schedule it for tomorrow
      if(java.lang.Math.abs(now.delta(nextEvent).getSeconds())<60)
      {
  ...
  /**StatusString value In represing the event time in the format HH:MM:SS.  Use this or the individual numbers*/
