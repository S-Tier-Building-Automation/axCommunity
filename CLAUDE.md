...
Known gap (maintainer follow-up): `release-please.yml` uses the default `GITHUB_TOKEN`, and PRs opened by `GITHUB_TOKEN` do **not** trigger `pull_request` workflows — so a freshly opened Release PR never gets CI Gate / PR Gate reported on it (a human edit/push re-fires them). Fix: create a GitHub App with `contents:write` + `pull_requests:write` on this repo, store its id/key as repo secrets, and generate a token via `actions/create-github-app-token` as the first step of `release-please.yml`, passing it as the action's `token:` input. Do **not** commit this workflow change before the secrets exist — the release train would fail on the missing secret.
