		...
		try
		{
			...
			try
			{
				if( !(type==null) )
				{
					...
					{
						if(getFacets().isNull())	{ return slot.getFacets();		}
	...
	private class threadedCalculate implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				...
				TableCursor	cursor	= (TableCursor)result.cursor();								// setup table cursor
				...
				while (cursor.next()) 
				{
					...
					try
					{
						...
						// is BSimple?
						if (((BObject) cursor.get(valueColumn)).isSimple())  
						{
							// handle result columns that are not complex type (aggregates for example)
							Value = new BStatusNumeric((BDouble.make(cursor.get(valueColumn).toString()).getDouble()));
						}
						else
						{
							// get BStatusNumeric from table
							Value = (BStatusNumeric) cursor.get(valueColumn);		
						}
