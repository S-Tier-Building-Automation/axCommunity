...
public class BTrendDynamicAnalyzer extends BComponent
{
  ...
  public void doExecute()
  {
    ...
    TableCursor localTableCursor = (TableCursor)localBITable.cursor();

    localTableCursor.next();

    double d1 = Double.parseDouble(localTableCursor.get(localColumnList.get(2)).toString());
    double d2 = Double.parseDouble(localTableCursor.get(localColumnList.get(3)).toString());
    double d3 = Double.parseDouble(localTableCursor.get(localColumnList.get(4)).toString());
    double d4 = Double.parseDouble(localTableCursor.get(localColumnList.get(5)).toString());
    double d5 = Double.parseDouble(localTableCursor.get(localColumnList.get(6)).toString());
  ...
  private static final BIcon icon = BIcon.make("local:|module://axCommunity/org/axcommunity/niagara/graphics/BAS_Logo.png");
  ...
  static
  {
    Class tmp163_160 = class$org$axcommunity$niagara$bql$BTrendDynamicAnalyzer; 
    if (tmp163_160 == null) tmp163_160=tmp163_160; 
    TYPE = Sys.loadType(BTrendDynamicAnalyzer.class$org$axcommunity$niagara$bql$BTrendDynamicAnalyzer = jdMethod_class("[Lorg.axcommunity.niagara.bql.BTrendDynamicAnalyzer;", false));
