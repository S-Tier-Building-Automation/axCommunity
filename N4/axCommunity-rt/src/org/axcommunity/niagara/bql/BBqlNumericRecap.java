...
public class BBqlNumericRecap extends BComponent {
	...
	public void calculate()
	{
		try
		{
			...
			TableCursor c = result.cursor();			            // setup table cursor
			...
			while (c.next())										// walk bql rows
			{
				...
				//is BSimple?
				if(((BObject)c.cell(valueColumn)).isSimple())  //handle result columns that are not complex type (aggregates for example)
				{			  
				  Value = new BStatusNumeric((BDouble.make(c.cell(valueColumn).toString()).getDouble()));
				}
				else	
			  {

				  Value = (BStatusNumeric) c.cell(valueColumn);		// get BStatusNumeric from table
			  }
